    ...
    def __init__(
        ...
        try:
            ...
            self.children = (
                ...
                Thread.run(
                    self.name + " waiter", self._monitor, please_stop=self.stopped, parent_thread=self,
                ),
            )
    ...
    def __init__(
        ...
        if not self.process:
            self.process = Process(
                "command shell", [cmd()], os_path(cwd), env, debug, shell, bufsize, timeout=60, parent_thread=threads.MAIN_THREAD
            )
