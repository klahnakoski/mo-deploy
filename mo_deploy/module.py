...
class Module(object):
        ...
        if travis_file.exists:
            ...
            for v in tested_versions:
                if v == "3.7":
                    ...
                    Log.error("Unhandled version {{version}}", version=v)
    ...
    def pypi(self):
        ...
        while not give_up_on_pypi:
            ...
            p, stdout, stderr = self.local(
                [self.python["latest"], "-m", "pip", "install", self.name + "=="], raise_on_error=False,
            )
    ...
    def run_tests(self, python_version, please_stop):
        ...
        with TempDirectory() as temp:
            ...
            if (self.directory / "tests" / "smoke_test.py").exists:
                self.local(
                    [python, "-Werror", "tests/smoke_test.py"], env={"PYTHONPATH": ""}, debug=True,
                )
            else:
                Log.warning("add tests/smoke_test.py to ensure the library will run after installed")
            ...
            with Timer("run tests"):
                ...
                Log.note("TESTS DONE")
                if len(stderr) < 2:
                    Log.error("Expecting unittest results (at least two lines of output)")
                num_tests = int(strings.between(
                    first(line for line in reversed(stderr) if line.startswith("Ran ")), "Ran ", " test",
                ))
                if num_tests == 0:
    ...
    def get_setup_version(self):
        return Version((self.directory / SETUPTOOLS).read_json(leaves=False).version, prefix="v",)
    ...
    def get_version(self):
        ...
        all_versions = self.all_versions = list(sorted(v for line in stdout for v in [Version(line)]))
    ...
    def get_old_dependencies(self, version):
        ...
        Log.note("Find {{name}}=={{version}} in git history", name=self.name, version=version)
    ...
    def please_upgrade(self):
        ignored_files = ["setup.py", "setuptools.json"]  # , ".travis.yml"]
        ...
        try:
            ...
            changes = count(
                l.strip() for l in stdout if l not in ignored_files and not any(l.startswith(d) for d in ignored_dir)
            )
            if changes:
                Log.note("{{num}} files changed", num=changes)
