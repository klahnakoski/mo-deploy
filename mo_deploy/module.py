...
class Module(object):
    ...
    def synch_travis_file(self):
        ...
        if travis_file.exists:
            ...
            travis_file.write(yaml.dump(
                from_data(content), default_flow_style=False, sort_keys=False
            ))
    ...
    def pypi(self):
        ...
        try:
            ...
            self.gen_setup_py_file()
            self.local(
                [self.python["latest"], "setup.py", "bdist_wheel", "--universal"], raise_on_error=True
            )
    ...
    def get_setup_version(self):
        return Version(
            (self.directory / SETUPTOOLS).read_json(leaves=False).version,
            prefix="v",
        )
    ...
    def please_upgrade(self):
        ...
        try:
            ...
            changes = count(
                l.strip()
                for l in stdout
                if l not in ignored_files and not any(l.startswith(d) for d in ignored_dir)
            )
            if changes:
