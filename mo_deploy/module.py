...
class Module(object):
    ...
    def deploy(self):
        ...
        try:
            ...
            while True:
                try:
                    self.local([self.git, "checkout", "-f", self.dev_branch], debug=True)
                    break
                except Exception as cause2:
                    if "unable to unlink old" in cause2:
                        continue
                    if "Another git process seems to be running in this repository" in cause2:
                        continue
                    raise
    ...
    def gen_setup_py_file(self):
        ...
        setup = File(self.directory / SETUPTOOLS).read_json(flexible=False)
        content = (
            "# encoding: utf-8\n"
            + "# THIS FILE IS AUTOGENERATED!\n"
            + "from setuptools import setup\n"
            + "setup(\n"
            + ",\n".join("    " + k + "=" + value2python(v) for k, v in setup.items() if exists(v))
            + "\n"
            + ")"
        )
    ...
    def last_deploy(self):
        logger.info("find last deployed version for {package}", package=self.package_name)
    ...
    def update_setup_json_file(self, new_version):
        ...
        setup = File(setup_json).read_json(flexible=False)
        ...
        if "3.13" not in self.test_versions:
            # ask user if they want to add 3.12
            value = input("Add 3.13 python to supported versions list? (y/N): ")
            if value in "yY":
                self.test_versions.append("3.13")
                setup.classifiers.append("Programming Language :: Python :: 3.13")
    ...
    def run_tests(self, python_version, please_stop):
        ...
        with TempDirectory() as temp:
            ...
            smoke_tests = [c for c in (self.directory/"tests").children if c.stem.startswith("smoke_test")]
            if smoke_tests:
                for smoke_test in smoke_tests:
                    self.local(
                        [python, "-Werror", smoke_test.os_path], env={"PYTHONPATH": ""}, debug=True, cwd=self.directory,
                    )
            else:
            ...
            with Timer("run tests"):
                process, stdout, stderr = self.local(
                    [python, "-m", "unittest", "discover", ".", "-v"], env={"PYTHONPATH": "."}, debug=True,
                )
    ...
    def get_old_dependencies(self, version):
        ...
        if p.returncode or any("invalid object name" in line for line in stderr):
            requirements = File(self.directory / SETUPTOOLS).read_json(flexible=False).install_requires
        else:
    ...
    def please_upgrade(self):
        ...
        while True:
            try:
                ...
                raise cause
        try:
            logger.info("check for changed files")
